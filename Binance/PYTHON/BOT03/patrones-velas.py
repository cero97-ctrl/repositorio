...
def check_confirmation(df, state, args):
    ...
    log_file = (
        args.trades_log_file.replace(".csv", "_backtest.csv")
        if args.backtest
        else args.trades_log_file
    )
...
def run_backtest(args):
    ...
    for i in range(
        ...
        if (
            ...
            and "Esperando vela de confirmación" not in signal_message
        ):
...
if __name__ == "__main__":
    # Esta estrategia no tiene argumentos adicionales, así que pasamos None.
    args, telegram_token, chat_id = utils.setup_logging_and_config(
        add_strategy_args_func=None
    )
