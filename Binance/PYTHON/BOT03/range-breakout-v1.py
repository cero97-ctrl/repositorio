...
def detect_rejection(df, level, direction, threshold):
    ...
    if total_range > 0:
        if (
            direction == "resistance"
            and row["high"] >= level
            and upper_wick / total_range > threshold
        ):
            return f"Rechazo en resistencia ({level:.2f})"
        elif (
            direction == "support"
            and row["low"] <= level
            and lower_wick / total_range > threshold
        ):
...
def evaluate_trade(df, args, log_file=None):
    ...
    in_range, low, high = detect_range(
        df, args.range_lookback, args.range_atr_multiplier
    )
    ...
    if state.get("in_range"):
        ...
        absorption_signal = detect_absorption(
            df, range_low, "support", args.absorption_ratio
        ) or detect_absorption(df, range_high, "resistance", args.absorption_ratio)
        rejection_signal = detect_rejection(
            df, range_high, "resistance", args.rejection_threshold
        ) or detect_rejection(df, range_low, "support", args.rejection_threshold)
    ...
    if state.get("in_range"):
        ...
        if (
            ...
            and latest["volume"] > avg_volume * args.volume_multiplier
        ):
...
if __name__ == "__main__":
    ...
    def add_strategy_args(parser):
        parser.add_argument(
            ...
            type=int,
            default=50,
            help="Periodo para detectar el rango.",
        )
        parser.add_argument(
            "--range-atr-multiplier",
            type=float,
            default=2.0,
            help="Multiplicador de ATR para definir el ancho máximo del rango.",
        )
        parser.add_argument(
            "--absorption-ratio",
            type=float,
            default=0.3,
            help="Ratio cuerpo/mecha para detectar absorción.",
        )
        parser.add_argument(
            "--rejection-threshold",
            type=float,
            default=0.7,
            help="Umbral de mecha/rango total para detectar rechazo.",
        )
        return parser

    args, telegram_token, chat_id = utils.setup_logging_and_config(
        add_strategy_args_func=add_strategy_args
    )
    ...
    logging.info(
        f"Lookback de Rango: {args.range_lookback} | Multiplicador ATR de Rango: {args.range_atr_multiplier}"
    )
    logging.info(
        f"Risk Stop Mult: {args.risk_stop_mult} | Risk TP Mult: {args.risk_tp_mult}"
    )
    logging.info("==========================================")
