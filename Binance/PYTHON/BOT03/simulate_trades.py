...
def run_simulation(
    ...
    """
    logging.info(
        f"Iniciando simulación con Balance Inicial=${initial_balance:,.2f}, Riesgo por Trade={risk_per_trade:.2%}"
    )
    logging.info(
        f"Límite de trades: {max_open_trades}, TPs Parciales: {partial_tp_count}, SL a BE: {trailing_sl_breakeven}"
    )
    logging.info(
        f"Trailing Stop Dinámico ATR: {'Activado (x' + str(trailing_sl_atr_mult) + ')' if trailing_sl_atr_mult > 0 else 'Desactivado'}"
    )
    ...
    if trades_df.empty:
        logging.warning(
            f"El archivo de señales '{trades_file}' está vacío. No se encontraron trades para simular."
        )
        ...
        header = trades_df.columns.tolist() + [
            "outcome",
            "pnl_percentage",
            "pnl_usd",
            "exit_price",
        ...
        pd.DataFrame(columns=header).to_csv(
            trades_file.replace(".csv", "_results.csv"), index=False
        )
        return

    # Convertir timestamps a datetime para poder comparar
    trades_df["timestamp"] = pd.to_datetime(trades_df["timestamp"])
    # --- CORRECCIÓN CRÍTICA: Estandarizar el formato de timestamp ---
    # Los bots guardan el timestamp como string. Al leerlo, pandas lo convierte.
    # Hacemos lo mismo con los datos históricos para que las fechas coincidan.
    ...
    for candle_index, candle in historical_df.iterrows():
        ...
        for i, pos in enumerate(open_positions):
            ...
            if trailing_sl_breakeven and not pos.get("sl_moved_to_be", False):
                ...
                if (
                    "LONG" in pos["type"].upper()
                    and candle["high"] >= pos["entry"] + risk_distance
                ):
                    pos["stop_loss"] = pos["entry"]
                    pos["sl_moved_to_be"] = True
                elif (
                    "SHORT" in pos["type"].upper()
                    and candle["low"] <= pos["entry"] - risk_distance
                ):
                    pos["stop_loss"] = pos["entry"]
                    pos["sl_moved_to_be"] = True

            # --- LÓGICA DE TRAILING STOP DINÁMICO (ATR) ---
            if trailing_sl_atr_mult > 0 and pos.get(
                "sl_moved_to_be", False
            ):  # Solo se activa si ya está en Break-Even
                if "LONG" in pos["type"].upper():
                    # Actualizar el precio más alto alcanzado
                    pos["highest_price_since_be"] = max(
                        pos.get("highest_price_since_be", pos["entry"]), candle["high"]
                    )
                    new_trailing_sl = pos["highest_price_since_be"] - (
                        pos["atr"] * trailing_sl_atr_mult
                    )
                    # Mover el SL solo si el nuevo es más alto que el actual
                    if new_trailing_sl > pos["stop_loss"]:
                        pos["stop_loss"] = new_trailing_sl
                elif "SHORT" in pos["type"].upper():
                    pos["lowest_price_since_be"] = min(
                        pos.get("lowest_price_since_be", pos["entry"]), candle["low"]
                    )
                    new_trailing_sl = pos["lowest_price_since_be"] + (
                        pos["atr"] * trailing_sl_atr_mult
                    )
                    if new_trailing_sl < pos["stop_loss"]:
                        pos["stop_loss"] = new_trailing_sl
            ...
            if "LONG" in pos["type"].upper():
                ...
                # Si no hay TPs parciales, la única salida con ganancia es por el Trailing Stop
                    ...
                    outcome, exit_price, close_reason = (
                        "WIN" if pos["stop_loss"] > pos["entry"] else "LOSS",
                        pos["stop_loss"],
                        "TRAILING_SL",
                    )
            ...
            if outcome:
                ...
                pnl_usd = (
                    (exit_price - pos["entry"]) * size_to_close
                    if "LONG" in pos["type"].upper()
                    else (pos["entry"] - exit_price) * size_to_close
                )
                pnl_percentage = (
                    (
                        pnl_usd
                        / (
                            pos["balance_at_entry"]
                            * (size_to_close / pos["original_size"])
                        )
                    )
                    * 100
                    if pos["balance_at_entry"] > 0 and pos["original_size"] > 0
                    else 0
                )
                ...
                logging.info(
                    f"  -> CERRADO PARCIALMENTE trade {closed_trade_record['id']}: {outcome} ({close_reason}), P&L: ${pnl_usd:,.2f}, Nuevo Balance: ${current_balance:,.2f}"
                )

                # Si la posición se ha cerrado completamente, marcarla para eliminar
                if (
                    pos["current_size"] < 1e-9 or close_reason == "SL"
                ):  # Usar una pequeña tolerancia para flotantes
                    positions_to_close_indices.append(i)
                    logging.info(
                        f"  -> POSICIÓN COMPLETA CERRADA para trade {pos['id']}."
                    )
        ...
        for i, order in enumerate(pending_orders):
            ...
            if stop_loss_breached_before_entry and not entry_price_reached:
                logging.warning(
                    f"  -> ORDEN CANCELADA {order['type']} de {order['timestamp']}. SL tocado antes de la entrada."
                )
                ...
                is_momentum_pattern = (
                    "3SOLDIERS" in order["type"].upper()
                    or "3CROWS" in order["type"].upper()
                )
                ...
                actual_entry_price = (
                    candle["open"] if is_momentum_pattern else order["entry"]
                )
                ...
                if is_momentum_pattern:
                    ...
                    if "LONG" in order["type"].upper():
                        order["stop_loss"] = (
                            actual_entry_price - original_risk_per_share
                        )
                        order["take_profit"] = actual_entry_price + (
                            original_risk_per_share * order["rr"]
                        )
                    else:  # SHORT
                        order["stop_loss"] = (
                            actual_entry_price + original_risk_per_share
                    ...
                    order["entry"] = (
                        actual_entry_price  # Actualizamos la orden con el precio real de entrada
                    )

                # --- LÓGICA PARA ABRIR LA POSICIÓN ---
                if len(open_positions) < max_open_trades:
                    capital_at_risk_in_open_trades = sum(
                        pos.get("risk_amount_usd", 0) for pos in open_positions
                    )
                    ...
                    position_size = (
                        risk_amount_usd / risk_per_share if risk_per_share > 0 else 0
                    )
                    ...
                    new_position.update(
                        {
                            ...
                            "original_stop_loss": order[
                                "stop_loss"
                            ],  # Guardamos el SL original para el cálculo de 1R
                    ...
                    logging.info(
                        f"-> ABIERTO nuevo trade {new_position['id']} ({new_position['type']}) en {candle['timestamp']}. Riesgo: ${risk_amount_usd:,.2f}"
                    )
                else:
                    logging.warning(
                        f"  -> OMITIDO trade {order['type']} en {candle['timestamp']}. Límite de {max_open_trades} posiciones abiertas alcanzado."
                    )

                orders_to_open_indices.append(
                    i
                )  # Marcar para eliminar de pendientes, ya sea abierta o omitida
        ...
        if candle["timestamp"] in trades_by_timestamp:
            for new_trade_signal in trades_by_timestamp[candle["timestamp"]]:
                ...
                logging.info(
                    f"  -> NUEVA ORDEN PENDIENTE ({new_trade_signal['type']}) registrada en {candle['timestamp']}. Esperando entrada en {new_trade_signal['entry']:.2f}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Archivo CSV con los trades a simular.",
    )
    parser.add_argument(
        "--data-file",
        type=str,
        required=True,
        help="Archivo CSV con los datos históricos de precios (ej: btc_1h_data.csv).",
    )
    parser.add_argument(
        "--initial-balance",
        type=float,
        default=10000,
        help="Balance inicial de la cuenta para la simulación.",
    )
    parser.add_argument(
        "--risk-per-trade",
        type=float,
        default=0.01,
        help="Porcentaje del capital a arriesgar por operación (ej: 0.01 para 1%).",
    )
    parser.add_argument(
        "--max-open-trades",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Número de Take Profits parciales a utilizar (0 = sin parciales, solo Trailing Stop).",
    )
    parser.add_argument(
        "--trailing-sl-breakeven",
        action="store_true",
        help="Activa el movimiento del Stop Loss a break-even después del primer TP parcial.",
    )
    parser.add_argument(
        "--trailing-sl-atr-mult",
        type=float,
        default=0.0,
        help="Multiplicador de ATR para el trailing stop dinámico (0 para desactivar).",
    )
    ...
    run_simulation(
        ...
        args.partial_tp_count,
        args.trailing_sl_breakeven,
        args.trailing_sl_atr_mult,
    )
