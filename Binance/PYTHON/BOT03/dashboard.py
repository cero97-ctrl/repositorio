...
# --- CORRECCIÓN: Usar la nueva función de configuración centralizada ---
args, _, _ = utils.setup_logging_and_config(add_strategy_args_func=None)

symbol = st.sidebar.text_input("Símbolo (Symbol)", args.symbol)
interval = st.sidebar.selectbox(
    "Intervalo (Interval)", ["1m", "5m", "15m", "30m", "1h", "4h", "1d"], index=4
)  # 1h por defecto
...
start_date = st.sidebar.date_input(
    "Fecha Inicio", value=end_date - timedelta(days=7)
...
refresh_interval = st.sidebar.number_input(
    "Intervalo (segundos)", value=60, min_value=10, max_value=3600
)
...
def load_data(symbol, interval, start_date_obj, end_date_obj, _args):
    ...
    start_time_ms = int(
        datetime.combine(start_date_obj, datetime.min.time()).timestamp() * 1000
    )
    end_time_ms = int(
        datetime.combine(end_date_obj, datetime.max.time()).timestamp() * 1000
    )
    ...
    df_full = utils.get_klines(
        symbol, interval, start_time_ms=fetch_start_time_ms, end_time_ms=end_time_ms
    )

    if not df_full.empty:
        # 3. Calcular los indicadores sobre el conjunto de datos completo (ej: 250 + 200 = 450 velas).
        df_full = utils.calculate_indicators(
            df_full, _args.volume_sma_period, _args.atr_window, _args.bollinger_window
        )
        ...
        if end_date_obj == datetime.now().date():
            ...
            if current_price and not df_full.empty:
                ...
                new_candle_series["timestamp"] = pd.to_datetime(
                    new_candle_series["close_time"] + 1, unit="ms"
                )
                new_candle_series["open"] = new_candle_series["close"]
                new_candle_series["high"] = max(
                    new_candle_series["high"], current_price
                )
        ...
        df_display = df_full[
            (df_full["timestamp"] >= pd.to_datetime(start_time_ms, unit="ms"))
            & (df_full["timestamp"] <= pd.to_datetime(end_time_ms, unit="ms"))
...
tab1, tab2, tab3 = st.tabs(
    ["📈 Gráfico en Vivo", "📊 Análisis de Backtest", "📜 Historial de Trades"]
)

with tab1:
    if df.empty:
        st.error(
            f"No se pudieron obtener datos para {symbol}. Revisa el símbolo o la conexión."
        )
    else:
        ...
        if os.path.exists(log_file_to_use):
            ...
            visible_end_date = pd.to_datetime(end_date + timedelta(days=1)).tz_localize(
                "UTC"
            )
            ...
            symbol_trades = trades_df[
                ...
                & (trades_df["timestamp"] < visible_end_date)
            ].copy()

            # Separar trades de compra (long) y venta (short)
            buy_trades = symbol_trades[
                symbol_trades["type"].str.contains("LONG", case=False)
            ].copy()
            sell_trades = symbol_trades[
                symbol_trades["type"].str.contains("SHORT", case=False)
            ].copy()
        ...
        fig.add_hline(
            y=70, line_dash="dot", line_color="red", line_width=1, row=3, col=1
        )
        fig.add_hline(
            y=30, line_dash="dot", line_color="green", line_width=1, row=3, col=1
        )
        ...
        st.dataframe(
            df.tail(10).iloc[::-1], use_container_width=True
        )  # Mostrar los 10 más recientes, invertidos para ver el último arriba
...
with tab2:
    ...
    if not result_files:
        st.warning(
            "No se encontraron archivos de resultados de backtest (ej: 'trades_log_results.csv')."
        )
        st.info(
            "Asegúrate de haber ejecutado el flujo de backtesting completo, incluyendo el script 'simulate_trades.py'."
        )
    else:
        ...
        selected_file = st.selectbox(
            "Selecciona un archivo de resultados para analizar:",
            result_files,
        ...
        if selected_file:
            ...
            default_selection = (
                default_signal_types if default_signal_types else list(signal_types)
            )
            ...
            filtered_df = filtered_df[
                filtered_df["outcome"] != "IN_PROGRESS"
            ].copy()  # Ignorar trades no cerrados
            ...
            if not filtered_df.empty:
                ...
                win_rate = (
                    (len(wins_df) / total_trades) * 100 if total_trades > 0 else 0
                )

                total_gain = wins_df["pnl_usd"].sum()
                total_loss = abs(losses_df["pnl_usd"].sum())
                profit_factor = (
                    total_gain / total_loss if total_loss > 0 else float("inf")
                )

                avg_win = wins_df["pnl_usd"].mean() if not wins_df.empty else 0
                avg_loss = (
                    abs(losses_df["pnl_usd"].mean()) if not losses_df.empty else 0
                )
                ...
                if "initial_balance" in results_df.columns:
                    ...
                    initial_balance_global = (
                        results_df.iloc[0]["balance_after_trade"]
                        - results_df.iloc[0]["pnl_usd"]
                    )

                net_pnl_perc = (
                    (net_pnl_usd / initial_balance_global) * 100
                    if initial_balance_global != 0
                    else 0
                )

                # Cálculo de Max Drawdown
                filtered_df["cummax_balance"] = (
                    initial_balance_global + filtered_df["pnl_usd"].cumsum()
                ).cummax()
                filtered_df["drawdown"] = (
                    filtered_df["cummax_balance"] - filtered_df["balance_after_trade"]
                ) / filtered_df["cummax_balance"]
                ...
                col4.metric(
                    "Balance Final Global",
                    f"${results_df.iloc[-1]['balance_after_trade']:,.2f}",
                )
                col4.metric(
                    "Máximo Drawdown", f"{max_drawdown:.2f}%", delta_color="inverse"
                )
                ...
                analysis_by_type["win_rate"] = (
                    analysis_by_type["wins"] / analysis_by_type["total_trades"]
                ) * 100
                analysis_by_type = analysis_by_type.sort_values(
                    by="pnl_usd", ascending=False
                )
                st.dataframe(
                    analysis_by_type,
                    use_container_width=True,
                    column_config={
                        "pnl_usd": st.column_config.NumberColumn(format="$%.2f"),
                ...
                filtered_df["direction"] = filtered_df["type"].apply(
                    lambda x: "LONG" if "LONG" in x else "SHORT"
                )
                analysis_by_direction = (
                ...
                analysis_by_direction["win_rate"] = (
                    analysis_by_direction["wins"]
                    / analysis_by_direction["total_trades"]
                ) * 100
                st.dataframe(
                    analysis_by_direction,
                    use_container_width=True,
                ...
                if not filtered_df.empty:
                    fig_hist = go.Figure(
                        data=[go.Histogram(x=filtered_df["pnl_usd"], nbinsx=50)]
                    )
            ...
            with st.expander("Gestión de Presets"):
                st.write(
                    "Guarda la configuración actual de filtros (archivo y tipos de señal) para usarla más tarde."
                )
                ...
                if st.button("Guardar Preset"):
                    if new_preset_name:
                        ...
                        st.success(
                            f"¡Preset '{new_preset_name}' guardado! Refrescando..."
                        )
                        st.rerun()
                    else:
                ...
                if not presets:
                    ...
                    preset_to_delete = st.selectbox(
                        "Selecciona un preset para eliminar:",
                    ...
                    if st.button("Eliminar Preset Seleccionado", type="primary"):
                        if preset_to_delete in presets:
                            ...
                            save_presets(presets)
                            st.success(
                                f"¡Preset '{preset_to_delete}' eliminado! Refrescando..."
                            )
                            st.rerun()
                        else:
...
with tab3:
    ...
    if os.path.exists(trades_log_file):
        ...
        st.dataframe(
            trades_df.tail(20).iloc[::-1], use_container_width=True
        )  # Mostrar los últimos 20 trades
    else:
