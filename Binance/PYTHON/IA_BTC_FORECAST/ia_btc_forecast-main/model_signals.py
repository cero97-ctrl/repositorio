...
def create_sequences(data, signals, seq_length):
    ...
    for i in range(len(data) - seq_length):
        ...
        y.append(
            signals[i + seq_length - 1]
        )  # La señal en el último punto de la secuencia
...
model.fit(
    X_train, y_train_cat, epochs=5, batch_size=1, validation_data=(X_test, y_test_cat)
)
...
plt.plot(
    prices.index[-len(y_test) :],
    scaler.inverse_transform(X_test[:, -1, 0].reshape(-1, 1)),
...
try:
    plt.scatter(
        prices.index[-len(y_test) :].to_numpy()[buy_signals],
        scaler.inverse_transform(X_test[buy_signals, -1, 0].reshape(-1, 1)),
        marker="^",
        color="green",
        label="Comprar",
        alpha=1,
...
try:
    plt.scatter(
        prices.index[-len(y_test) :].to_numpy()[sell_signals],
        scaler.inverse_transform(X_test[sell_signals, -1, 0].reshape(-1, 1)),
        marker="v",
        color="red",
        label="Vender",
        alpha=1,
