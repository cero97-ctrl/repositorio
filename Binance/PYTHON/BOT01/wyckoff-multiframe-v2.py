...
def detect_htf_phase(df_htf, args):
    ...
    if is_in_range:
        ...
        volume_on_lows = (
            support_tests_df["volume"].mean() if not support_tests_df.empty else 0
        )

        resistance_tests_df = recent_df[recent_df["high"] >= tr_high * 0.98]
        volume_on_highs = (
            resistance_tests_df["volume"].mean() if not resistance_tests_df.empty else 0
        )
        ...
        # Lógica de Distribución: Precio en la parte alta del rango con volumen decreciente en los testeos.
            ...
            return (
                "range_contested"  # Rango sin una clara dominancia de oferta o demanda
            )
        ...
        if (
            latest_close > latest_candle["EMA_50"]
            and latest_candle["EMA_50"] > latest_candle["EMA_200"]
        ):
            return "markup"
        elif (
            latest_close < latest_candle["EMA_50"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Bot Wyckoff profesional multi-timeframe v2 (Análisis Estructural)"
    )
    parser.add_argument(
        "--wyckoff",
        action="store_true",
    ...
    parser.add_argument(
        "--htf-lookback",
        type=int,
        default=int(os.getenv("HTF_LOOKBACK", 100)),
        help="Velas a mirar en HTF para análisis estructural",
    )
    parser.add_argument(
        "--htf-range-thresh",
        type=float,
        default=float(os.getenv("HTF_RANGE_THRESH", 0.10)),
        help="Umbral porcentual para definir un rango en HTF",
    )
    parser.add_argument(
        "--htf-range-atr-mult",
        type=float,
        default=float(os.getenv("HTF_RANGE_ATR_MULT", 3.0)),
        help="Multiplicador de ATR para definir el tamaño de un rango en HTF",
    )
    ...
    logging.info("==========================================")
    logging.info(
        "Iniciando Bot Wyckoff Multi-Timeframe v2 (con análisis estructural HTF)"
    )
    ...
    logging.info(
        f"HTF Lookback: {args.htf_lookback} | HTF Range ATR Multiplier: {args.htf_range_atr_mult}"
    )
    logging.info("==========================================")
