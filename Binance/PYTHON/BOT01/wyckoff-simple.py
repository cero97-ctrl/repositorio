...
def check_confirmation(df, state, args):
    ...
    utils.clear_state()
    if trade_type:
        entry, stop, tp, rr = utils.compute_risk_levels(
            latest["close"],
            latest["ATR"],
            trade_type,
            args.risk_stop_mult,
...
def evaluate_trade(df, args, log_file=None):
    ...
    if utils.is_hammer(
        ...
        entry, stop_loss, take_profit, rr = utils.compute_risk_levels(
            latest["close"],
            latest["ATR"],
            "long",
            args.risk_stop_mult,
        ...
        signal_text = (
            f"🕯️ Martillo detectado | Entrada: {entry:.8f} | ATR: {latest['ATR']:.8f}\n"
        )
        ...
        if entry and stop_loss and take_profit:
            signal_text = utils.format_risk_management_message(
                signal_text, entry, stop_loss, take_profit, rr
            )
    ...
    if args.wyckoff:
        ...
        if ev_type == "SPRING":
            signals.append(
                ev_msg
            )  # ev_msg ya viene formateado con la gestión de riesgo
            utils.record_trade(
                log_file if log_file else args.trades_log_file,
                args.symbol,
                "LONG_WYCKOFF_SPRING",
...
def run_backtest(args):
    logging.info(
        f"Iniciando backtest con simulación de trades desde: {args.backtest_file}"
    )
    try:
    ...
    for i in range(
        ...
        evaluate_trade(
            sub_df, args, backtest_log_file
        )  # Pasamos el nombre del archivo de log
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Wyckoff activo: {args.wyckoff} | Vol Mult: {args.wyckoff_volume_mult} | ATR Thresh: {args.wyckoff_atr_thresh}"
    )
