# patrones-velas-v2.py (Bot enfocado en patrones de velas + filtro de volatilidad ATR + filtro de confluencia EMA)
...
def check_confirmation(df, state, args):
    ...
    if pattern == "hammer" and latest["close"] > previous["high"]:
        entry, sl, tp, rr = utils.compute_risk_levels(
            latest["close"],
            latest["ATR"],
            "long",
            args.risk_stop_mult,
        ...
        message = utils.format_risk_management_message(
            f"✅ Confirmación alcista para patrón {pattern.upper()}", entry, sl, tp, rr
        )
        utils.record_trade(
            args.trades_log_file,
            args.symbol,
            f"CONFIRMED_LONG_{pattern.upper()}",
            entry,
            sl,
            tp,
            latest["ATR"],
        ...
        message = utils.format_risk_management_message(
            f"✅ Confirmación bajista para patrón {pattern.upper()}", entry, sl, tp, rr
        )
        utils.record_trade(
            args.trades_log_file,
            args.symbol,
            f"CONFIRMED_SHORT_{pattern.upper()}",
            entry,
            sl,
            tp,
            latest["ATR"],
...
def evaluate_trade(df, args, log_file=None):
    ...
    atr_mean = utils.get_atr_mean_for_volatility(
        df
    )  # Usamos 50 como en wyckoff para consistencia
    ...
    # --- NUEVO: Filtro de Confluencia con EMAs ---
    # Definimos "cerca" como que el precio toque la EMA o esté a un 0.2% de distancia.
    proximity_threshold = 0.002
    near_ema_50_support = (
        latest["low"] <= latest["EMA_50"] * (1 + proximity_threshold)
    ) and (latest["close"] > latest["EMA_50"])
    near_ema_200_support = (
        latest["low"] <= latest["EMA_200"] * (1 + proximity_threshold)
    ) and (latest["close"] > latest["EMA_200"])
    near_ema_50_resistance = (
        latest["high"] >= latest["EMA_50"] * (1 - proximity_threshold)
    ) and (latest["close"] < latest["EMA_50"])
    near_ema_200_resistance = (
        latest["high"] >= latest["EMA_200"] * (1 - proximity_threshold)
    ) and (latest["close"] < latest["EMA_200"])
    ...
    if (
        ...
        and (near_ema_50_support or near_ema_200_support)
    ):
        signal_text = "🕯️ Hammer detectado"
        if latest["close"] <= latest["Boll_Lower"]:
            signal_text += " en Banda de Bollinger 📉"
        if poc_zone:
        ...
        formatted_signal = utils.format_risk_management_message(
            signal_text, entry, sl, tp, rr
        )
        signals.append(formatted_signal)
        if entry:
            utils.record_trade(
                args.trades_log_file if log_file is None else log_file,
                args.symbol,
                "LONG_HAMMER",
                entry,
                sl,
    ...
    if (
        ...
        and is_not_oversold
        and (near_ema_50_resistance or near_ema_200_resistance)
    ):
    ...
    if (
        utils.is_bullish_engulfing(latest, previous)
        and is_not_overbought
        and (near_ema_50_support or near_ema_200_support)
    ):
    ...
    if (
        utils.is_bearish_engulfing(latest, previous)
        and is_not_oversold
        and (near_ema_50_resistance or near_ema_200_resistance)
    ):
...
def run_backtest(args):
    logging.info(
        f"Iniciando backtest de detección de señales desde: {args.backtest_file}"
    )
    try:
...
if __name__ == "__main__":
    ...
    logging.info(
        f"POC: {args.poc} | ATR Window: {args.atr_window} | Bollinger Window: {args.bollinger_window}"
    )
