...
linear_model = Sequential(
    [LSTM(50, return_sequences=True, input_shape=(df.shape[0], 1)), LSTM(50), Dense(1)]
)
...
stock_model = Sequential(
    [Dense(16, input_shape=(len(X_stock_columns),)), Dense(4), Dense(1)]
)

merged_layer = Concatenate()(
    [linear_model.outputs[0], seq_model.outputs[0], stock_model.outputs[0]]
)
model = Model(
    [linear_model.inputs[0], seq_model.inputs[0], stock_model.inputs[0]], merged_layer
)
...
X_seq_data, y_seq_data = create_sequences(
    df["y_price"].values.reshape(-1, 1), seq_length
)
...
for i in range(n_records_train, (df.shape[0] - seq_length - n_days_predict * 2) + 1):
    ...
    X_seq_train = seq_scaler.fit_transform(X_seq_train.reshape(-1, 1)).reshape(
        i, seq_length, 1
    )
    X_seq_test = seq_scaler.transform(X_seq_test.reshape(-1, 1)).reshape(
        n_days_predict, seq_length, 1
    )
    X_seq_val = seq_scaler.transform(X_seq_val.reshape(-1, 1)).reshape(
        n_days_predict, seq_length, 1
    )
    ...
    predictions = linear_scaler.inverse_transform(
        np.array([predictions.mean()]).reshape(-1, 1)
    )
    ...
    ax[0].plot(
        df.index[-len(full_y_val_descaled) :],
        full_y_val_descaled,
        color="blue",
        label="Precio Real",
    )
    ax[0].plot(
        df.index[-len(full_predictions) :],
        full_predictions,
        color="red",
        label="Predicción",
    )
    ...
    ax[1].plot(
        df.index[-len(full_y_val_descaled) :], full_y_val_descaled - full_predictions
    )
    plt.xlabel("Fecha")
...
current_price = np.array(
    [linear_scaler.inverse_transform(X_linear_val)[-1][0], response.json()["price"]]
).astype(np.float32)
