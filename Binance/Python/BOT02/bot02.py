...
def detect_absorption(df, level, direction='support'):
    ...
    for i, row in recent.iterrows():
        ...
        if direction == 'support' and row['low'] <= level and lower_wick > 0 and body / lower_wick < ABSORPTION_RATIO:
            signals.append((i, 'Absorción en soporte'))
        elif direction == 'resistance' and row['high'] >= level and upper_wick > 0 and body / upper_wick < ABSORPTION_RATIO:
...
def detect_rejection(df, level, direction='resistance'):
    ...
    for i, row in recent.iterrows():
        ...
        if total_range > 0 and direction == 'resistance' and row['high'] >= level and upper_wick / total_range > REJECTION_THRESHOLD:
            signals.append((i, 'Rechazo en resistencia'))
        elif total_range > 0 and direction == 'support' and row['low'] <= level and lower_wick / total_range > REJECTION_THRESHOLD:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            df = get_klines(SYMBOL, INTERVAL, limit=RANGE_LOOKBACK + ATR_PERIOD + VOLUME_AVG_PERIOD)
            ...
            logging.info(f"Ciclo completado. Esperando {sleep_time} segundos para la próxima vela...")
