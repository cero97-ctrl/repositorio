...
def get_klines(symbol, interval, limit=None, start_time_ms=None, end_time_ms=None):
    ...
    if start_time_ms is not None and end_time_ms is not None:
        ...
        logging.info(
            f"Obteniendo datos desde {datetime.fromtimestamp(start_time_ms/1000)} hasta {datetime.fromtimestamp(end_time_ms/1000)}"
        )
        ...
        logging.error(
            "Debe proporcionar 'limit' o un rango de fechas ('start_time_ms' y 'end_time_ms')."
        )
        return pd.DataFrame()
...
def calculate_indicators(df, volume_sma_period, atr_window, boll_window):
    ...
    logging.info(
        "Indicadores calculados: EMAs, RSI, MACD, Volumen SMA, ATR, Bollinger Bands."
    )
    return df
...
def compute_risk_levels(
    entry_price, atr, direction="long", stop_mult=1.5, tp_mult=2.5, sl_buffer=0.0
):
    """
    Calcula stop_loss, take_profit y risk:reward (rr) dependiendo de la dirección.
    Retorna (stop_loss, take_profit, rr).
    """
    if pd.isna(atr) or atr <= 0 or entry_price is None:
        return None, None, None, None  # Añadido None para entry_price

    # --- LÓGICA DE RIESGO UNIFICADA ---
    # El riesgo se define por la distancia al stop loss.
    if direction == "long":
        # El SL se calcula restando el ATR o un buffer. Priorizamos el ATR si stop_mult > 0.
        stop_loss = (
    ...
    rr = (
        round((take_profit - entry_price) / risk, 2)
        if direction == "long"
        else round((entry_price - take_profit) / risk, 2)
    )

    return entry_price, round(stop_loss, 8), round(take_profit, 8), rr


def format_risk_management_message(
    signal_text, entry_price, stop_loss, take_profit, rr_ratio
):
...
def is_three_white_soldiers(df_slice, body_ratio_threshold=0.3):
    ...
    c1_body_ok = (c1["close"] - c1["open"]) > (
        (c1["high"] - c1["low"]) * body_ratio_threshold
    )
    c2_body_ok = (c2["close"] - c2["open"]) > (
        (c2["high"] - c2["low"]) * body_ratio_threshold
    )
    c3_body_ok = (c3["close"] - c3["open"]) > (
        (c3["high"] - c3["low"]) * body_ratio_threshold
    )

    return (
        are_all_bullish
        and are_closes_higher
        and c1_body_ok
        and c2_body_ok
        and c3_body_ok
    )
...
def is_three_black_crows(df_slice, body_ratio_threshold=0.3):
    ...
    c1_body_ok = (c1["open"] - c1["close"]) > (
        (c1["high"] - c1["low"]) * body_ratio_threshold
    )
    c2_body_ok = (c2["open"] - c2["close"]) > (
        (c2["high"] - c2["low"]) * body_ratio_threshold
    )
    c3_body_ok = (c3["open"] - c3["close"]) > (
        (c3["high"] - c3["low"]) * body_ratio_threshold
    )

    return (
        are_all_bearish
        and are_closes_lower
        and c1_body_ok
        and c2_body_ok
        and c3_body_ok
    )
...
def ensure_trades_log_exists(file_path):
    ...
    if not os.path.exists(file_path):
        with open(file_path, "w") as f:
            ...
            f.write(
                "timestamp,symbol,type,entry,stop_loss,take_profit,atr,rr,notes,outcome,pnl_percentage,exit_price,exit_time\n"
            )
...
def record_trade(
    ...
    line = (
        f"{ts},{symbol},{ttype},{entry},{stop_loss},{take_profit},{atr},{rr},{notes}\n"
    )
    with open(file_path, "a") as f:
        f.write(line)
    logging.info(
        f"Trade registrado en {file_path}: {symbol} {ttype} entry={entry} rr={rr}"
    )


# === FUNCIONES COMUNES PARA BOTS ===
def execute_single_run_common(
    args, telegram_token, chat_id, evaluate_trade_func, check_confirmation_func
):
    """
    ...
    df = calculate_indicators(
        df, args.volume_sma_period, args.atr_window, args.bollinger_window
    )
    ...
    message = (
        f"--- Análisis para {args.symbol} ({args.interval}) ---\n\n{signal_message}"
    )
    logging.info(message)

    # Solo enviar a Telegram si hay una señal activa y no es solo un mensaje de espera o baja volatilidad
    if (
        "⏳" not in signal_message
        and "Volatilidad baja" not in signal_message
        and "❌ Sin confirmación" not in signal_message
    ):
        send_telegram_message(message, telegram_token, chat_id, pre_escaped=True)


def run_bot_main_loop(
    args,
    telegram_token,
    chat_id,
    evaluate_trade_func,
    check_confirmation_func,
    ...
    send_telegram_message(
        startup_message_text, telegram_token, chat_id, pre_escaped=True
    )
    logging.info("Mensaje de inicio enviado a Telegram.")

    while True:
        try:
            execute_single_run_common(
                args,
                telegram_token,
                chat_id,
                evaluate_trade_func,
            ...
            logging.info(
                f"Análisis completado. Esperando {args.sleep} segundos para el próximo ciclo."
            )
...
# === CONFIGURACIÓN ===
def setup_logging_and_config(add_strategy_args_func=None):
    """
    Función centralizada para cargar configuración, parsear argumentos y configurar logging.
    """
    load_dotenv()
    ...
    parser.add_argument(
        "--volume-sma-period", type=int, default=int(os.getenv("VOLUME_SMA_PERIOD", 20))
    )
    parser.add_argument(
        "--hammer-multiplier",
        type=float,
        default=float(os.getenv("HAMMER_MULTIPLIER", 2.0)),
    )
    parser.add_argument(
        "--shooting-star-multiplier",
        type=float,
        default=float(os.getenv("SHOOTING_STAR_MULTIPLIER", 2.0)),
    )
    parser.add_argument(
        "--volume-multiplier",
        type=float,
        default=float(os.getenv("VOLUME_MULTIPLIER", 1.5)),
    )
    parser.add_argument(
        "--atr-window", type=int, default=int(os.getenv("ATR_WINDOW", 14))
    )
    parser.add_argument(
        "--bollinger-window", type=int, default=int(os.getenv("BOLLINGER_WINDOW", 20))
    )
    parser.add_argument("--poc", type=float, default=float(os.getenv("POC", 0.0)))

    # Parámetros de gestión de riesgo y Wyckoff
    parser.add_argument(
        "--risk-stop-mult", type=float, default=float(os.getenv("RISK_STOP_MULT", 1.5))
    )
    parser.add_argument(
        "--risk-tp-mult", type=float, default=float(os.getenv("RISK_TP_MULT", 2.5))
    )
    parser.add_argument(
        "--trades-log-file",
        type=str,
        default=os.getenv("TRADES_LOG_FILE", "trades_log.csv"),
    )
    parser.add_argument(
        "--wyckoff",
        action="store_true",
        default=str(os.getenv("WYCKOFF", "false")).lower() in ("true", "1", "t"),
    )
    parser.add_argument(
        "--wyckoff-volume-mult",
        type=float,
        default=float(os.getenv("WYCKOFF_VOLUME_MULT", 1.3)),
    )
    parser.add_argument(
        "--wyckoff-atr-thresh",
        type=float,
        default=float(os.getenv("WYCKOFF_ATR_THRESH", 1.1)),
    )

    # Parámetros para Wyckoff Multi-Timeframe (v1, v2, v3)
    parser.add_argument(
        "--wyckoff-professional",
        action="store_true",
        default=str(os.getenv("WYCKOFF_PROFESSIONAL", "false")).lower()
        in ("true", "1", "t"),
    )
    parser.add_argument(
        "--htf-interval", type=str, default=os.getenv("HTF_INTERVAL", "4h")
    )
    parser.add_argument(
        "--htf-limit", type=int, default=int(os.getenv("HTF_LIMIT", 120))
    )
    parser.add_argument(
        "--htf-vol-mult", type=float, default=float(os.getenv("HTF_VOL_MULT", 1.2))
    )
    # Añadimos los de v2 y v3 para que no den error en el futuro
    parser.add_argument(
        "--htf-lookback", type=int, default=int(os.getenv("HTF_LOOKBACK", 100))
    )
    parser.add_argument(
        "--htf-range-thresh",
        type=float,
        default=float(os.getenv("HTF_RANGE_THRESH", 0.10)),
    )
    parser.add_argument(
        "--htf-climactic-vol-mult",
        type=float,
        default=float(os.getenv("HTF_CLIMACTIC_VOL_MULT", 2.5)),
    )
    parser.add_argument(
        "--htf-test-vol-mult",
        type=float,
        default=float(os.getenv("HTF_TEST_VOL_MULT", 0.8)),
    )
    parser.add_argument(
        "--htf-breakout-vol-mult",
        type=float,
        default=float(os.getenv("HTF_BREAKOUT_VOL_MULT", 1.5)),
    )
    ...
    parser.add_argument(
        "--rr-ratio",
        type=float,
        default=float(os.getenv("RR_RATIO", 2.0)),
        help="Ratio Riesgo/Beneficio para Take Profit (usado en v6)",
    )
    parser.add_argument(
        "--sl-buffer",
        type=float,
        default=float(os.getenv("SL_BUFFER", 0.002)),
        help="Buffer porcentual para Stop Loss (usado en v6)",
    )
    ...
    parser.add_argument(
        "--backtest",
        action="store_true",
        default=str(os.getenv("BACKTEST", "false")).lower() in ("true", "1", "t"),
    )
    parser.add_argument(
        "--backtest-file",
        type=str,
        default=os.getenv("BACKTEST_FILE", "historical_data.csv"),
    )

    # --- MEJORA: Permitir que la estrategia añada sus propios argumentos ---
    if add_strategy_args_func:
        parser = add_strategy_args_func(parser)

    args = parser.parse_args()
    telegram_token = os.getenv("TELEGRAM_TOKEN", "").strip()
    chat_id = os.getenv("TELEGRAM_CHAT_ID", "").strip()

    # Configurar logging después de parsear los argumentos
    logging.basicConfig(
