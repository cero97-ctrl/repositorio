...
def evaluate_trade(df, args, log_file=None):
    ...
    near_ema_50_support = (latest['low'] <= latest['EMA_50'] * (1 + proximity_threshold)) and (latest['close'] > latest['EMA_50'])
    near_ema_200_support = (latest['low'] <= latest['EMA_200'] * (1 + proximity_threshold)) and (latest['close'] > latest['EMA_200'])
    near_ema_50_resistance = (latest['high'] >= latest['EMA_50'] * (1 - proximity_threshold)) and (latest['close'] < latest['EMA_50'])
    near_ema_200_resistance = (latest['high'] >= latest['EMA_200'] * (1 - proximity_threshold)) and (latest['close'] < latest['EMA_200'])

    if utils.is_hammer(latest['open'], latest['close'], latest['high'], latest['low'], args.hammer_multiplier) and is_not_overbought and (near_ema_50_support or near_ema_200_support):
    ...
    if utils.is_shooting_star(latest['open'], latest['close'], latest['high'], latest['low'], args.shooting_star_multiplier) and is_not_oversold and (near_ema_50_resistance or near_ema_200_resistance):
    ...
    if utils.is_bullish_engulfing(latest, previous) and is_not_overbought and (near_ema_50_support or near_ema_200_support):
    ...
    if utils.is_bearish_engulfing(latest, previous) and is_not_oversold and (near_ema_50_resistance or near_ema_200_resistance):
