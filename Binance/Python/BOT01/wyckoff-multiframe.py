...
def detect_htf_phase(df_htf, args):
    ...
    if (
        df_htf["EMA_50"].iloc[-1] > df_htf["EMA_200"].iloc[-1]
        and df_htf["EMA_50"].iloc[-2] < df_htf["EMA_200"].iloc[-2]
    ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Bot Wyckoff profesional multi-timeframe + gestión de riesgo básica"
    )
    ...
    parser.add_argument(
        "--htf-range-thresh",
        type=float,
        default=float(os.getenv("HTF_RANGE_THRESH", 0.03)),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--htf-slope-thresh",
        type=float,
        default=float(os.getenv("HTF_SLOPE_THRESH", 0.0005)),
    )
    ...
    logging.info(
        f"Wyckoff active: {args.wyckoff} | vol mult: {args.wyckoff_volume_mult} | atr thresh: {args.wyckoff_atr_thresh}"
    )
    logging.info(
