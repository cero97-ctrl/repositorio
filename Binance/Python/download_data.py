...
def download_historical_data(symbol, interval, limit, output_file):
    ...
    try:
        ...
        if not df.empty:
            ...
            logging.info(f"✅ Datos guardados exitosamente en '{output_file}'. Se descargaron {len(df)} velas.")
        else:
            logging.warning("No se recibieron datos de Binance. El DataFrame está vacío.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbol", type=str, required=True, help="Símbolo del par a descargar (ej: BTCUSDT).")
    parser.add_argument("--interval", type=str, required=True, help="Intervalo de las velas (ej: 1h, 4h, 1d).")
    parser.add_argument("--limit", type=int, default=500, help="Número de velas a descargar.")
    parser.add_argument("--output", type=str, required=True, help="Nombre del archivo CSV de salida.")
