...
def is_secondary_test(candle, tr_support, args):
    ...
    is_testing_support = abs(candle['low'] - tr_support) / tr_support < 0.02 # Testeo dentro del 2%
...
def detect_htf_phase(df_htf, args):
    ...
    if current_phase == "SEARCHING":
        if is_selling_climax(latest, previous, args):
            ...
            logging.info(f"[HTF State] Selling Climax detectado. Cambiando a ACCUMULATION_PHASE_A. Soporte inicial en {htf_state['sc_low']:.2f}")
    ...
    # --- FASE A DE ACUMULACIÓN ---
        ...
        if not df_after_sc.empty:
            ...
            logging.info(f"[HTF State] Automatic Rally detectado. Cambiando a ACCUMULATION_PHASE_B. Resistencia en {ar_high:.2f}")
            save_htf_state(htf_state)
            return htf_state['phase']
    ...
    # --- FASE B DE ACUMULACIÓN ---
        ...
        if is_secondary_test(latest, htf_state['tr_support'], args):
            logging.info(f"[HTF State] Secondary Test exitoso en {latest['low']:.2f} con bajo volumen. Se confirma Acumulación.")
    ...
    # --- FASE DE MARKUP ---
        ...
        if latest['EMA_50'] < latest['EMA_200']:
            logging.info("[HTF State] Cruce bajista de EMAs. Finalizando Markup. Volviendo a SEARCHING.")
...
def evaluate_trade(df, args, log_file=None):
    ...
    if args.wyckoff_professional:
        ...
        if df_htf.empty:
            ...
            df_htf['timestamp'] = pd.to_datetime(df_htf['timestamp'], unit='ms') # Asegurar datetime
    ...
    if utils.is_hammer(latest['open'], latest['close'], latest['high'], latest['low'], args.hammer_multiplier):
        ...
        if poc_zone: signal_text += f"- En ZONA DE SOPORTE POC (${args.poc:.2f}) 🔥\n"
        if sl and tp_p: signal_text = utils.format_risk_management_message(signal_text, entry_p, sl, tp_p, rr)
        
        if args.wyckoff_professional:
            if htf_phase in valid_long_phases:
                signals.append(signal_text)
                utils.record_trade(file_path=log_file or args.trades_log_file,
                                   symbol=args.symbol,
                                   ttype='LONG',
                                   entry=entry_p,
                                   stop_loss=sl,
                ...
                logging.info(f"Martillo detectado pero HTF no compatible ({htf_phase}), descartado.")
        else:
            signals.append(signal_text)
            # Aplicar la misma mejora aquí y en el resto de llamadas a record_trade
            utils.record_trade(log_file or args.trades_log_file, args.symbol, 'LONG', entry_p, sl, tp_p, latest['ATR'], rr, notes='hammer', timestamp=latest['timestamp'])
            pending_state = {"pattern": "hammer", "price": entry_p}
    ...
    if utils.is_shooting_star(latest['open'], latest['close'], latest['high'], latest['low'], args.shooting_star_multiplier):
        ...
        if poc_zone: signal_text += f"- En ZONA DE RESISTENCIA POC (${args.poc:.2f}) ⚠️\n"
        if sl and tp_p: signal_text = utils.format_risk_management_message(signal_text, entry_p, sl, tp_p, rr)

        if args.wyckoff_professional:
            if htf_phase in valid_short_phases:
                signals.append(signal_text)
                utils.record_trade(log_file or args.trades_log_file, args.symbol, 'SHORT', entry_p, sl, tp_p, latest['ATR'], rr, 'shooting_star', latest['timestamp'])
                pending_state = {"pattern": "shooting_star", "price": entry_p}
            else:
                logging.info(f"Estrella Fugaz detectada pero HTF no compatible ({htf_phase}), descartado.")
        else:
            signals.append(signal_text)
            utils.record_trade(log_file or args.trades_log_file, args.symbol, 'SHORT', entry_p, sl, tp_p, latest['ATR'], rr, 'shooting_star', latest['timestamp'])
            pending_state = {"pattern": "shooting_star", "price": entry_p}
    ...
    if ev_type:
        if args.wyckoff_professional:
            if ev_type == 'SPRING' and htf_phase in valid_long_phases:
                ...
                utils.record_trade(log_file or args.trades_log_file, args.symbol, 'LONG_WYCKOFF', entry_wyckoff, stop_wyckoff, tp_wyckoff, ev_atr, ev_rr, 'wyckoff_spring', latest['timestamp'])
                pending_state = {"pattern": "wyckoff_spring", "price": entry_wyckoff}
            elif ev_type == 'UPTHRUST' and htf_phase in valid_short_phases:
                signals.append(ev_msg)
                utils.record_trade(log_file or args.trades_log_file, args.symbol, 'SHORT_WYCKOFF', entry_wyckoff, stop_wyckoff, tp_wyckoff, ev_atr, ev_rr, 'wyckoff_upthrust', latest['timestamp'])
                pending_state = {"pattern": "wyckoff_upthrust", "price": entry_wyckoff}
            else:
                logging.info(f"Evento Wyckoff {ev_type} detectado pero HTF no compatible ({htf_phase}), descartado.")
        else:
            signals.append(ev_msg)
            trade_type_note = 'wyckoff_spring' if ev_type == 'SPRING' else 'wyckoff_upthrust'
            utils.record_trade(log_file or args.trades_log_file, args.symbol, f'{ev_type}_WYCKOFF', entry_wyckoff, stop_wyckoff, tp_wyckoff, ev_atr, ev_rr, trade_type_note, latest['timestamp'])
...
def run_backtest(args):
    ...
    if os.path.exists(backtest_log_file): os.remove(backtest_log_file)
...
if __name__ == "__main__":
    ...
    logging.info("==========================================")
    logging.info("Iniciando Bot Wyckoff Multi-Timeframe v3 (con Máquina de Estados HTF)")
    logging.info(f"Símbolo: {args.symbol} | Intervalo: {args.interval}")
    logging.info(f"Wyckoff PROFESSIONAL: {args.wyckoff_professional} | HTF: {args.htf_interval}")
    logging.info(f"HTF Vol Multipliers: Climactic={args.htf_climactic_vol_mult}, Test={args.htf_test_vol_mult}, Breakout={args.htf_breakout_vol_mult}")
    logging.info("==========================================")
    ...
    utils.clear_state()
    clear_htf_state()
    logging.info("Estados (LTF y HTF) limpiados. Iniciando en modo de operación en vivo.")
